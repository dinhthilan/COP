# -*- coding: utf-8 -*-
"""
Created on Tue Dec 22 15:16:45 2020
...
n = 20
m = 4
x0 = np.array([2,3,5,5,1,2,7,3,6,10,2,2,6,15,1,2,1,2,1,3]).reshape(20,1)
...
def obj_new(x):
    val,grad=w3.obj(x.reshape(n,1))
    return val,grad.reshape(n,)

def cons_new(x):
    vec_val,mat_grad=w3.cons(x.reshape(n,1))
...
s=1
print('s = ',s)
_,_,_,_,FEAS4,VAL4 = PDS.run(A,b,x0,v,lambd,m,w3.obj,w3.cons,
...
s=1.5
print('s = ',s)
_,_,_,_,FEAS5,VAL5 = PDS.run(A,b,x0,v,lambd,m,w3.obj,w3.cons,
                 Print=True,history=True,delta=delta,rho=1/s,s =s,numiter=numiter,
                 eps=eps)
...
s=2
print('s = ',s)
_,_,_,_,FEAS6,VAL6 = PDS.run(A,b,x0,v,lambd,m,w3.obj,w3.cons,
                 Print=True,history=True,delta=delta,rho=1/s,s =s,numiter=numiter,
                 eps=eps)
