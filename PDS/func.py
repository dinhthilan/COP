...
def eval_funcx1(x):
    return x[0][0],np.array([1,0,0]).reshape(3,1)
...
def eval_funcx2(x):
    return x[1][0],np.array([0,1,0]).reshape(3,1)



def eval_funcx3(x):
    return x[2][0],np.array([0,0,1]).reshape(3,1)
...
def eval_f_ineq(x):
    ...
    vec_val,mat_grad=eval_sum(func1,func2,x)
    return vec_val.reshape(1,1),mat_grad
